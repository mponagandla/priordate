...
// In-Memory Query Cache & In-Flight Promise Deduplication Layer
...
/**
 * Caches asynchronous Supabase queries and deduplicates concurrent in-flight requests.
 */
async function fetchWithCache<T>(
  ...
  const promise = fetcher()
    .then((result) => {
      queryCache.set(cacheKey, { data: result, timestamp: Date.now(), ttlMs });
...
// 1. Fetch distinct classification categories for Cohort Finder (Cached for 10 min)
export async function getClassificationOptions(): Promise<string[]> {
  return fetchWithCache('classifications', 10 * 60 * 1000, async () => {
    try {
...
// 2. Fetch Cohort Stats for specified category, date, and country (Cached for 5 min)
export async function getCohortAnalysis(
  classification: string,
  priorityDateString: string, // e.g. "2022-10-15" or "2022-10"
  country: string = 'All Other Countries'
) {
  const cacheKey = `cohort:${classification}:${priorityDateString}:${country}`;
  return fetchWithCache(cacheKey, 5 * 60 * 1000, async () => {
    try {
...
// 3. Search employers for autocomplete (Cached for 2 min)
export async function searchEmployers(query: string): Promise<Employer[]> {
  if (!query || query.trim().length < 2) return [];
  const cleanQuery = query.trim().toLowerCase();

  return fetchWithCache(`employer_search:${cleanQuery}`, 2 * 60 * 1000, async () => {
    try {
      const { data, error } = await supabase
        .from('employers')
        .select('*')
...
// 4. Get Employer Profile Details (Cached for 5 min)
export async function getEmployerProfile(employerId: string) {
  const cleanSlug = employerId.toLowerCase().trim();

  return fetchWithCache(`employer_profile:${cleanSlug}`, 5 * 60 * 1000, async () => {
    try {
...
// 5. Get Trends Time Series Data (Cached for 5 min)
export async function getTrendsData(category: string = 'EB-2 PERM', dateRange: string = '5y') {
  return fetchWithCache(`trends:${category}:${dateRange}`, 5 * 60 * 1000, async () => {
    try {
...
// 6. Get Data Sources and Ingestion Logs for Methodology & Data Badges (Cached for 5 min)
export async function getDataSourcesWithLogs(): Promise<DataSource[]> {
  return fetchWithCache('data_sources_with_logs', 5 * 60 * 1000, async () => {
    try {
